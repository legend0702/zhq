...
public final class ResolvableType {
	...
	private boolean isAssignableFrom(ResolvableType other,
		...
		if (exactMatch ? !ourResolved.equals(otherResolved) : !ClassUtils
	...
	public ResolvableType[] getInterfaces() {
		Class<?> resolved = resolve();
		if (resolved == null
				|| ArraysUtils.isEmpty(resolved.getGenericInterfaces())) {
	...
	private Type resolveBounds(Type[] bounds) {
		if (ArraysUtils.isEmpty(bounds) || Object.class.equals(bounds[0])) {
			return null;
		}
		return bounds[0];
	}
	...
	private ResolvableType resolveVariable(TypeVariable<?> variable) {
		...
		if (this.type instanceof ParameterizedType) {
			...
			for (int i = 0; i < variables.length; i++) {
				if (ObjectUtils.nullSafeEquals(variables[i].getName(),
						variable.getName())) {
	...
	public String toString() {
		...
		if (hasGenerics()) {
			result.append('<');
			result.append(StringUtils.collectionToDelimitedString(
	...
	public boolean equals(Object obj) {
		...
		return (ObjectUtils.nullSafeEquals(this.type, other.type)
				&& variableResolverSourceEquals(other.variableResolver) && ObjectUtils
	...
	private boolean variableResolverSourceEquals(VariableResolver other) {
		...
		return ObjectUtils.nullSafeEquals(this.variableResolver.getSource(),
				other.getSource());
	}
