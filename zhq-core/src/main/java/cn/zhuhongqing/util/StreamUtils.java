...
import static cn.zhuhongqing.util.Assert.isTrue;
...
public class StreamUtils {
	 ...
	 */
	public static <InElementT, OutElementT> Spliterator<OutElementT> map(Spliterator<InElementT> fromSpliterator,
	 ...
	 */
	public static <InElementT, OutElementT> Spliterator<OutElementT> flatMap(
		...
		isTrue((topCharacteristics & Spliterator.SUBSIZED) == 0,
				"flatMap does not support SUBSIZED characteristic");
		isTrue((topCharacteristics & Spliterator.SORTED) == 0, "flatMap does not support SORTED characteristic");
		notNull(fromSpliterator);
		notNull(function);
		return new FlatMapSpliteratorOfObject<InElementT, OutElementT>(null, fromSpliterator, function,
				topCharacteristics, topSize);
	}
	 ...
	 * function} returns {@code null} for an input, it is replaced with an empty
	 * stream.)
	 */
	public static <InElementT> Spliterator.OfInt flatMapToInt(Spliterator<InElementT> fromSpliterator,
	 ...
	 * stream.)
	 */
	public static <InElementT> Spliterator.OfLong flatMapToLong(Spliterator<InElementT> fromSpliterator,
	 ...
	 * stream.)
	 */
	public static <InElementT> Spliterator.OfDouble flatMapToDouble(Spliterator<InElementT> fromSpliterator,
	 ...
	 */
	abstract static class FlatMapSpliterator<InElementT, OutElementT, OutSpliteratorT extends Spliterator<OutElementT>>
		...
		FlatMapSpliterator(OutSpliteratorT prefix, Spliterator<InElementT> from,
				Function<? super InElementT, OutSpliteratorT> function, 
		 ...
		 * overloads of these methods, not overrides. They are annotated @Override
		 * because they implement methods from Spliterator.OfPrimitive (and override
		 * default implementations from Spliterator.OfPrimitive or a subtype like
		 * Spliterator.OfInt).
		 */
	...
	/**
	 * Implementation of {@link Stream#flatMap} with an object spliterator output
	 * type.
	 *
	 * <p>
	 * To avoid having this type, we could use {@code FlatMapSpliterator} directly.
	 * The main advantages to having the type are the ability to use its constructor
	 * reference below and the parallelism with the primitive version. In short, it
	 * makes its caller ({@code flatMap}) simpler.
	 *
	 * @param <InElementT>  the element type of the input spliterator
	 * @param <OutElementT> the element type of the output spliterators
	 */
	static final class FlatMapSpliteratorOfObject<InElementT, OutElementT>
		...
		FlatMapSpliteratorOfObject(Spliterator<OutElementT> prefix, Spliterator<InElementT> from,
	...
	/**
	 * Implementation of {@link Stream#flatMap} with a primitive spliterator output
	 * type.
	 *
	 * @param <InElementT>      the element type of the input spliterator
	 * @param <OutElementT>     the (boxed) element type of the output spliterators
	 * @param <OutConsumerT>    the specialized consumer type for the primitive
	 *                          output type
	 * @param <OutSpliteratorT> the primitive spliterator type associated with
	 *                          {@code OutElementT}
	 */
	abstract static class FlatMapSpliteratorOfPrimitive<InElementT, OutElementT, OutConsumerT, OutSpliteratorT extends Spliterator.OfPrimitive<OutElementT, OutConsumerT, OutSpliteratorT>>
		...
		FlatMapSpliteratorOfPrimitive(OutSpliteratorT prefix, Spliterator<InElementT> from,
	...
	static final class FlatMapSpliteratorOfInt<InElementT>
		...
		FlatMapSpliteratorOfInt(Spliterator.OfInt prefix, Spliterator<InElementT> from,
				Function<? super InElementT, Spliterator.OfInt> function, int characteristics, long estimatedSize) {
	...
	static final class FlatMapSpliteratorOfLong<InElementT>
		...
		FlatMapSpliteratorOfLong(Spliterator.OfLong prefix, Spliterator<InElementT> from,
				Function<? super InElementT, Spliterator.OfLong> function, int characteristics, long estimatedSize) {
	...
	static final class FlatMapSpliteratorOfDouble<InElementT>
		...
		FlatMapSpliteratorOfDouble(Spliterator.OfDouble prefix, Spliterator<InElementT> from,
				Function<? super InElementT, Spliterator.OfDouble> function, int characteristics, long estimatedSize) {
