...
public class URIUtils implements SchemeAndProtocol {
	...
	public static URI toURI(String pathOrURI) {
		Path p = null;
		try {
			p = PathUtils.toPath(pathOrURI);
		} catch (Exception e) {
			// Solve file path like 'file:c:/user'
			int index = pathOrURI.indexOf(FILE_PATH_PREFIX);
			if (index != -1) {
				p = PathUtils.toPath(pathOrURI.substring(FILE_PATH_PREFIX.length(), pathOrURI.length()));
			}
		}
		// Is fileSystem?
		if (GeneralUtils.isNotNull(p) && p.isAbsolute()) {
			return p.toUri();
		}
	 ...
	 * @param path
	 *            like "cn/zhuhongqing/utils/URIUtils.class"
	 * 
