...
import cn.zhuhongqing.utils.ConcurrentReferenceHashMap;
 ...
 * 
 * public void example() {
 * 	ResolvableType t = ResolvableType.forField(getClass().getDeclaredField(
 * 			&quot;myMap&quot;));
...
public final class ResolvableType {
	...
	private boolean isAssignableFrom(ResolvableType other,
		...
		if (this.type instanceof TypeVariable) {
			...
			if (this.variableResolver != null) {
				ResolvableType resolved = this.variableResolver
			...
			if (ourResolved == null) {
				...
				if (other.variableResolver != null) {
					ResolvableType resolved = other.variableResolver
							.resolveVariable(variable);
					if (resolved != null) {
						ourResolved = resolved.resolve();
						checkGenerics = false;
		...
		if (exactMatch ? !ourResolved.equals(otherResolved) : !ClassUtil
		...
		if (checkGenerics) {
			...
			for (int i = 0; i < ourGenerics.length; i++) {
				if (!ourGenerics[i].isAssignableFrom(typeGenerics[i],
						matchedBefore)) {
					return false;
				}
			}
	...
	public boolean isArray() {
		...
		return (((this.type instanceof Class && ((Class<?>) this.type)
				.isArray())) || this.type instanceof GenericArrayType || resolveType()
				.isArray());
	}
	...
	public ResolvableType[] getInterfaces() {
		Class<?> resolved = resolve();
		if (resolved == null
				|| ArraysUtil.isEmpty(resolved.getGenericInterfaces())) {
		...
		if (this.type instanceof WildcardType) {
			...
			if (wt.getLowerBounds().length == 0) {
				...
				if (upperBounds.length == 0
						|| (upperBounds.length == 1 && Object.class
		...
		for (int i = 2; i <= nestingLevel; i++) {
			if (result.isArray()) {
				...
				Integer index = (typeIndexesPerLevel != null ? typeIndexesPerLevel
						.get(i) : null);
				index = (index == null ? result.getGenerics().length - 1
						: index);
	...
	public ResolvableType[] getGenerics() {
		...
		if (this.generics == null) {
			if (this.type instanceof Class<?>) {
				...
				Type[] actualTypeArguments = ((ParameterizedType) this.type)
	...
	private Class<?> resolveClass() {
		...
		if (this.type instanceof GenericArrayType) {
			...
			return (resolvedComponent != null ? Array.newInstance(
					resolvedComponent, 0).getClass() : null);
		}
	...
	ResolvableType resolveType() {
		...
		if (this.type instanceof WildcardType) {
			Type resolved = resolveBounds(((WildcardType) this.type)
					.getUpperBounds());
			if (resolved == null) {
				resolved = resolveBounds(((WildcardType) this.type)
						.getLowerBounds());
			}
		...
		if (this.type instanceof TypeVariable) {
			...
			// Fallback to bounds
			return forType(resolveBounds(variable.getBounds()),
					this.variableResolver);
		}
		return NONE;
	}
	...
	private Type resolveBounds(Type[] bounds) {
		if (ArraysUtil.isEmpty(bounds) || Object.class.equals(bounds[0])) {
			return null;
		}
		return bounds[0];
	}
	...
	private ResolvableType resolveVariable(TypeVariable<?> variable) {
		...
		if (this.type instanceof ParameterizedType) {
			...
			for (int i = 0; i < variables.length; i++) {
				if (ObjectUtil.nullSafeEquals(variables[i].getName(),
						variable.getName())) {
	...
	public String toString() {
		...
		if (hasGenerics()) {
			result.append('<');
			result.append(StringUtil.collectionToDelimitedString(
					Arrays.asList(getGenerics()), ", "));
			result.append('>');
		}
	...
	public boolean equals(Object obj) {
		...
		return (ObjectUtil.nullSafeEquals(this.type, other.type)
				&& variableResolverSourceEquals(other.variableResolver) && ObjectUtil
					.nullSafeEquals(this.componentType, other.componentType));
	}

	@Override
	public int hashCode() {
		return ObjectUtil.nullSafeHashCode(this.type);
	}
	...
	private boolean variableResolverSourceEquals(VariableResolver other) {
		...
		return ObjectUtil.nullSafeEquals(this.variableResolver.getSource(),
				other.getSource());
	}

	private static ResolvableType[] forTypes(Type[] types,
	...
	/**
	 * Return a {@link ResolvableType} for the specified {@link Class}. For
	 * example {@code ResolvableType.forClass(MyArrayList.class)}.
	 * 
	 * @param sourceClass
	...
	public static ResolvableType forField(Field field) {
		return forType(null, field.getGenericType(), null);
	}
	...
	public static ResolvableType forField(Field field,
			Class<?> implementationClass) {
		ResolvableType owner = forType(implementationClass).as(
				field.getDeclaringClass());
		return forType(null, field.getGenericType(), owner.asVariableResolver());
	}
	...
	public static ResolvableType forField(Field field,
		...
		implementationType = (implementationType == null ? NONE
				: implementationType);
	...
	public static ResolvableType forField(Field field, int nestingLevel) {
		return forType(null, field.getGenericType(), null).getNested(
				nestingLevel);
	}
	...
	public static ResolvableType forArrayComponent(ResolvableType componentType) {
		Class<?> arrayClass = Array.newInstance(componentType.resolve(), 0)
				.getClass();
	...
	public static ResolvableType forClassWithGenerics(Class<?> sourceClass,
		...
		return forType(sourceClass, new TypeVariablesVariableResolver(
				typeVariables, generics));
	}
	 ...
	 *         {@link VariableResolver}
	 */
	static ResolvableType forType(Type type, Type typeProvider,
		...
		// Check the cache, we may have a ResolvableType that may have already
		// been resolved
		cache.purgeUnreferencedEntries();
		ResolvableType key = new ResolvableType(type, variableResolver);
		ResolvableType resolvableType = cache.get(key);
		if (resolvableType == null) {
			resolvableType = new ResolvableType(type, variableResolver, null);
	...
	@SuppressWarnings("serial")
	private static class TypeVariablesVariableResolver implements
			VariableResolver {
		...
		public TypeVariablesVariableResolver(TypeVariable<?>[] typeVariables,
				ResolvableType[] generics) {
	...
	private static class WildcardBounds {
		...
		public static WildcardBounds get(ResolvableType type) {
			...
			Kind boundsType = (wildcardType.getLowerBounds().length > 0 ? Kind.LOWER
					: Kind.UPPER);
			Type[] bounds = boundsType == Kind.UPPER ? wildcardType
					.getUpperBounds() : wildcardType.getLowerBounds();
