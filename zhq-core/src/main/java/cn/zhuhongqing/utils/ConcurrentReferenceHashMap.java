...
public class ConcurrentReferenceHashMap<K, V> extends AbstractMap<K, V>
	...
	public V get(Object key) {
		Reference<K, V> reference = getReference(key,
				Restructure.WHEN_NECESSARY);
	 ...
	 */
	protected final Reference<K, V> getReference(Object key,
	...
	public boolean remove(Object key, final Object value) {
		return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER,
				TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(Reference<K, V> reference,
					Entry<K, V> entry) {
				if (entry != null
						&& ObjectUtil.nullSafeEquals(entry.getValue(), value)) {
					reference.release();
					return true;
				}
				return false;
	...
	public boolean replace(K key, final V oldValue, final V newValue) {
		return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_BEFORE,
				TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(Reference<K, V> reference,
					Entry<K, V> entry) {
				if (entry != null
						&& ObjectUtil.nullSafeEquals(entry.getValue(), oldValue)) {
	...
	protected final class Segment extends ReentrantLock {
		...
		public Segment(int initialCapacity) {
			...
			this.initialSize = 1 << calculateShift(initialCapacity,
					MAXIMUM_SEGMENT_SIZE);
			setReferences(createReferenceArray(this.initialSize));
		}

		public Reference<K, V> getReference(Object key, int hash,
				Restructure restructure) {
		...
		protected final void restructureIfNecessary(boolean allowResize) {
			...
			if ((reference != null) || (needsResize && allowResize)) {
				...
				try {
					...
					if (allowResize && needsResize
							&& restructureSize < MAXIMUM_SEGMENT_SIZE) {
						restructureSize <<= 1;
						resizing = true;
					}
					...
					for (int i = 0; i < this.references.length; i++) {
						...
						while (reference != null) {
							if (!toPurge.contains(reference)
									&& (reference.get() != null)) {
								int index = getIndex(reference.getHash(),
										restructured);
								restructured[index] = this.referenceManager
										.createReference(reference.get(),
												reference.getHash(),
												restructured[index]);
							}
		...
		private Reference<K, V> findInChain(Reference<K, V> reference,
				Object key, int hash) {
			while (reference != null) {
	...
	protected static final class Entry<K, V> implements Map.Entry<K, V> {
		...
		public final boolean equals(Object other) {
			...
			return (ObjectUtil.nullSafeEquals(getKey(), otherEntry.getKey()) && ObjectUtil
					.nullSafeEquals(getValue(), otherEntry.getValue()));
		}

		@Override
		public final int hashCode() {
			return (ObjectUtil.nullSafeHashCode(this.key) ^ ObjectUtil
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(Object o) {
			if (o != null && o instanceof Map.Entry<?, ?>) {
				...
				if (other != null) {
					return ObjectUtil.nullSafeEquals(entry.getValue(),
							other.getValue());
				}
			}
			return false;
	...
	protected class ReferenceManager {
		...
		public Reference<K, V> createReference(Entry<K, V> entry, int hash,
			...
			if (ConcurrentReferenceHashMap.this.referenceType == ReferenceType.WEAK) {
				return new WeakEntryReference<K, V>(entry, hash, next,
						this.queue);
			}
	...
	private static final class SoftEntryReference<K, V> extends
		...
		public SoftEntryReference(Entry<K, V> entry, int hash,
				Reference<K, V> next, ReferenceQueue<Entry<K, V>> queue) {
			super(entry, queue);
			this.hash = hash;
