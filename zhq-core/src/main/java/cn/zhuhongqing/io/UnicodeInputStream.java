...
public class UnicodeInputStream extends InputStream {
	...
	/**
	 * Returns detected UTF encoding or <code>null</code> if no UTF encoding has
	 * been detected (i.e. no BOM). If stream is not read yet, it will be
	...
	public static final byte[] BOM_UTF32_BE = new byte[] { (byte) 0x00,
			(byte) 0x00, (byte) 0xFE, (byte) 0xFF };
	public static final byte[] BOM_UTF32_LE = new byte[] { (byte) 0xFF,
	...
	protected void init() throws IOException {
		...
		if (targetEncoding == null) {
			...
			if ((bom[0] == BOM_UTF32_BE[0]) && (bom[1] == BOM_UTF32_BE[1])
					&& (bom[2] == BOM_UTF32_BE[2])
					&& (bom[3] == BOM_UTF32_BE[3])) {
				encoding = "UTF-32BE";
				unread = n - 4;
			} else if ((bom[0] == BOM_UTF32_LE[0])
					&& (bom[1] == BOM_UTF32_LE[1])
					&& (bom[2] == BOM_UTF32_LE[2])
					&& (bom[3] == BOM_UTF32_LE[3])) {
				encoding = "UTF-32LE";
				unread = n - 4;
			} else if ((bom[0] == BOM_UTF8[0]) && (bom[1] == BOM_UTF8[1])
					&& (bom[2] == BOM_UTF8[2])) {
				encoding = "UTF-8";
				unread = n - 3;
			} else if ((bom[0] == BOM_UTF16_BE[0])
					&& (bom[1] == BOM_UTF16_BE[1])) {
				encoding = "UTF-16BE";
				unread = n - 2;
			} else if ((bom[0] == BOM_UTF16_LE[0])
					&& (bom[1] == BOM_UTF16_LE[1])) {
				encoding = "UTF-16LE";
				unread = n - 2;
			} else {
			...
			if (targetEncoding.equals("UTF-8")) {
				...
				bom = BOM_UTF16_LE;
			} else if (targetEncoding.equals("UTF-16BE")
					|| targetEncoding.equals("UTF-16")) {
				bom = BOM_UTF16_BE;
			} else if (targetEncoding.equals("UTF-32LE")) {
				bom = BOM_UTF32_LE;
			} else if (targetEncoding.equals("UTF-32BE")
					|| targetEncoding.equals("UTF-32")) {
				bom = BOM_UTF32_BE;
			} else {
