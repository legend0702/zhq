...
public class ReferenceMap<K, V> extends AbstractMap<K, V> implements
	...
	private V execute(Strategy strategy, K key, V value) {
		...
		return (V) strategy.execute(this, keyReference,
	...
	private Object dereference(ReferenceType referenceType, Object reference) {
		return referenceType == STRONG ? reference : ((Reference<?>) reference)
				.get();
	}
	...
	private enum PutStrategy implements Strategy {
			...
			public Object execute(ReferenceMap<?, ?> map, Object keyReference,
				...
				do {
					...
					do {
						...
						existingValue = map
								.dereferenceValue(existingValueReference);
					...
					if (map.delegate.replace(keyReference,
							existingValueReference, valueReference)) {
		...
		PUT_IF_ABSENT {
			public Object execute(ReferenceMap<?, ?> map, Object keyReference,
				...
				Object existingValue;
				do {
					existingValueReference = map.delegate.putIfAbsent(
